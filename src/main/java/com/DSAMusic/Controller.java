...
public class Controller {
    ...
    private void initialize() throws Exception {
        ...
        createPlaylistButton.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
        ...
        File selectedDirectory = new File("C:\\Users\\Duaa Syed\\Music");
        songTable.setItems(songsUrls(selectedDirectory));
    ...
    public ObservableList<Song> songsUrls(File dir) throws Exception {
        List<Song> songData = new LinkedList<>();
            ...
            if (file.isFile()) {
                ...
                if (name.endsWith("mp3") || name.endsWith("wav")) {
                    try {
                        ...
                        Song song = new Song(String.valueOf(i), tag.getArtist(), name, kbToMb(file.length()), secToMin(mp3.getLengthInSeconds()), tag.getAlbum(), file.getAbsolutePath());
        ...
        setImage();
        return FXCollections.observableList(songData);
    }
    ...
    public void playPauseSong(Song song) throws Exception{
        if(song != null) {
            ...
            volumeSlider.setStyle("-jfx-track-color: black; -jfx-thumb-color: #2c3e50;");
